...
public class Biblioteca
{
    ...
    public boolean prestarLibro(Calendar p_fechaRetiro, Socio p_socio, Libro p_libro){
        ...
        if (this.getSocios().contains(p_socio) && this.getLibros().contains(p_libro)){
            ...
            System.out.println("safdsadfsaf");
            return true;}
        else{ return false;}
    ...
    public ArrayList<Prestamo> prestamosVencidos(){
        ...
        for(Libro libro: this.getLibros()){
            try{
                if(libro.getPrestamo().vencido(fechaHoy)){
                    prestamos.add(libro.getPrestamo());
                }
            }catch(Exception e){
                return prestamos;
            }
        ...
        for(Socio socio: this.getSocios()){
            if(socio.soyDeLaClase().equals("Docente")){
                ...
                if(docente.esResponsable()){
                    listaDocResp += socio.toString();
                    listaDocResp += "|| Libros Prestados: " + socio.cantLibrosPrestados() + "\n";
            }
            }
        }
        return listaDocResp;
